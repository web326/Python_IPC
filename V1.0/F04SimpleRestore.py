...
class action():
    ...
    def simplerestore(self):
        ...
        time.sleep(0.5)
        self.driver.find_element_by_xpath(".//*[@id='diMaintain']/div[9]/span[2]/input").click()
        time.sleep(0.5)
...
class SimpleRestore(unittest.TestCase,action):
    def setUp(self):
        print '简单恢复初始化开始'
    def tearDown(self): 
        #self.driver.quit()
...
def test_simplerestore():
    suite=unittest.TestSuite(unittest.makeSuite(SimpleRestore))
    unittest.TextTestRunner(verbosity=2).run(suite)
 
def test05():
    threads = []
    t1 = threading.Thread(target=test_simplerestore,args=())
    threads.append(t1)
